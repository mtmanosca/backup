...
public class UploadEmployeeScheduleUploadingController extends MasterController<EmployeeScheduleUploadingMain> {
	...
	List<Date> cutoffDatesList = new ArrayList<>();
	...
	public void extractBiometricsData() {
		...
		c.setTime(this.cutoffFrom);
		Date dateStart = this.cutoffFrom;
		do {
			this.cutoffDatesList.add(dateStart);
			c.add(Calendar.DATE, 1);
			dateStart = c.getTime();
		} while (this.cutoffTo.getTime() >= dateStart.getTime());
				...
				try {
					...
					if (biometricsDataList != null && !biometricsDataList.isEmpty()) {
						...
						if (empIdList != null && !empIdList.isEmpty()) {
							...
							counter = 0;
							for (Employee employee : mainApplication.getEmployeeObservableList()) {

								List<EmployeeScheduleUploading> employeeScheduleUploadingList = new ArrayList<>();
								employeeScheduleUploadingList.addAll(employeeScheduleUploadingWithRegularSchedList
										.stream().filter(p -> p.getEmployeeId().equals(employee.getEmployeeCode()))
										.collect(Collectors.toList()));

								if (!employeeScheduleUploadingList.isEmpty()) {
									setRegularSchedule(employeeScheduleUploadingList);
								}
							...
							// REGULAR SCHED OLD PROCESS
							// for (EmployeeScheduleUploading employeeScheduleUploading :
							// employeeScheduleUploadingWithRegularSchedList) {
	...
	public void setListToBeSave(EmployeeScheduleUploading employeeScheduleUploading) {
		...
		// List<EmployeeScheduleUploading> existingDataList = new ArrayList<>();
		// existingDataList.addAll(this.mainApplication.getDataByEmployeeIdPayFrom(employeeIdEntry,
		// this.payFrom));
		...
		boolean isDateAlreadySaved = this.existingDataList.stream()
				.filter(p -> p.getEmploymentHistory().getEmployee().getEmployeeCode().equals(employeeIdEntry)
						&& p.getPayFrom().equals(this.payFrom) && p.getPayTo().equals(this.payTo)
						&& p.getDateEntry().compareTo(dateEntryToBeSave) == 0)
				.findFirst().isPresent();

		// EmployeeScheduleUploading isDateAlreadySaved = this.mainApplication
		// .getDataByEmpIdDateEntryPayFrom(employeeIdEntry, dateEntryToBeSave,
		// this.payFrom);
		...
		// List<EmployeeScheduleUploading> existingDataPerEmployeePerPayPeriodList = new
		// ArrayList<>();
		// existingDataPerEmployeePerPayPeriodList
		// .addAll(this.mainApplication.getDataByEmployeeIdPayFrom(employeeIdEntry,
		// this.payFrom));

		List<EmployeeScheduleUploading> existingDataPerEmployeePerPayPeriodList = this.existingDataList.stream()
				.filter(p -> p.getEmploymentHistory().getEmployee().getEmployeeCode().equals(employeeIdEntry)
						&& p.getPayFrom().equals(this.payFrom) && p.getPayTo().equals(this.payTo))
				.collect(Collectors.toList());

		if (!isDateAlreadySaved) {
			if (!isDateEntryExistInNextPayPeriod && !isDateEntryExistInPrevPayPeriod) {
	...
	public void setRegularSchedule(List<EmployeeScheduleUploading> employeeScheduleUploadingList) {
		...
		// bioPerEmployeeByDatesList.addAll(employeeScheduleUploadingList.stream()
		// .filter(p ->
		// p.getDateEntry().equals(employeeScheduleUploading.getDateEntry()))
		// .collect(Collectors.toList()));

		for (Date date : this.cutoffDatesList) {
			bioPerEmployeeByDatesList.clear();

			bioPerEmployeeByDatesList.addAll(employeeScheduleUploadingList.stream()
					.filter(p -> p.getDateEntry().equals(date)).collect(Collectors.toList()));
			...
			if (bioPerEmployeeByDatesList != null && !bioPerEmployeeByDatesList.isEmpty()) {
				String dayOfDate = bioPerEmployeeByDatesList.get(0).getDayOfDate();

				EmployeeScheduleUploading employeeScheduleUploadingToSave = bioPerEmployeeByDatesList.get(0);

				// timeIn = employeeScheduleUploadingToSave;
				...
				ScheduleEncoding scheduleEncodingImported = this.mainApplication.getScheduleEncodingMain()
						...
						.getScheduleImportedByEmpIdAndDay(employeeScheduleUploadingToSave.getEmployeeId(), dayOfDate);
				...
				if (scheduleEncodingImported != null) {
					...
					employeeScheduleEncoding = this.mainApplication.getEmployeeScheduleEncodingMain()
							.getEmployeeScheduleByEmployeeID(
									employeeScheduleUploadingToSave.getEmployee().getEmployeeCode());
				...
				if (employeeScheduleEncoding != null || scheduleEncodingImported != null) {
					...
					// }
					//
					// if ((bioEntry.getTime().equals(lunchOutReference.getTime())
					// || bioEntry.getTime().after(lunchOutReference.getTime()))
					// && bioEntry.getTime().before(lunchInReference.getTime())) {
					// lunchEntryList.add(entry);
					// }
					//
					// }
					// if (bioEntry.getTime().before(timeInReferenceEnd.getTime())
					// && bioEntry.getTime().before(lunchTime)) {
					// // if (bioEntry.getTime().before(timeInReferenceEnd.getTime())
					// // && bioEntry.getTime().before(timeInReference.getTime())) {
					// if (!this.listTimeInEntries.contains(entry)) {
					// this.listTimeInEntries.add(entry);
					...
					if (scheduleEncoding != null) {
						...
						timeIn = this.listTimeInEntries.isEmpty() ? bioPerEmployeeByDatesList.get(0)
								: this.listTimeInEntries.get(0);
						timeOut = bioPerEmployeeByDatesList.get(bioPerEmployeeByDatesList.size() - 1);

						if (!isBreakExempted) {

							Collections.sort(lunchEntryList, new Comparator<EmployeeScheduleUploading>() {
								@Override
								public int compare(EmployeeScheduleUploading o1, EmployeeScheduleUploading o2) {
									return o1.getTimeEntry().compareTo(o2.getTimeEntry());
								}
							});

							if (!lunchEntryList.isEmpty()) {
							...
							EmployeeScheduleUploading result[] = this.setTimeEntriesWithLunch(timeIn, timeOut, lunchIn,
									lunchOut, employeeScheduleUploadingToSave, timeInReferenceEnd, timeInReference,
							...
							EmployeeScheduleUploading[] results = this.setTimeEntriesWithoutLunch(timeIn, timeOut,
									...
									employeeScheduleUploadingToSave);
				...
				if (this.setFinalBiometricsData(employeeScheduleUploadingToSave, timeIn, timeOut, lunchOut, lunchIn)) {
					continue;
				}

				// add in regular schedule list
				this.mainApplication.getListFinalizeBiometricsOnlyBio().add(employeeScheduleUploadingToSave);

				// for (EmployeeScheduleUploading employeeScheduleUploadingByEmployeeAndDate :
				// bioPerEmployeeByDatesList) {}
			}
		}
		...
		// // finalize biometrics data and check if already added in regular schedule
		// list
		// if (this.setFinalBiometricsData(employeeScheduleUploading, timeIn, timeOut,
		// lunchOut, lunchIn)) {
		// return;
		// }
		//
				...
				if (bioPerEmployeeByDatesSortedList != null && !bioPerEmployeeByDatesSortedList.isEmpty()) {
					...
					for (EmployeeScheduleUploading employeeScheduleUploading : bioPerEmployeeByDatesSortedList) {
						...
						if (scheduleEncodingList != null && !scheduleEncodingList.isEmpty()) {
							...
							boolean isBreakExempted = scheduleEncoding.getIsBreakExempted() == 1 ? true : false;
							...
							for (EmployeeScheduleUploading entry : bioPerEmployeeByDatesSortedList) {
								Time bioEntryDate = entry.getTimeEntry();
								Calendar bioEntry = Calendar.getInstance();
								bioEntry.setTime(bioEntryDate);
							...
							timeIn = this.listTimeInEntries.isEmpty() ? bioPerEmployeeByDatesSortedList.get(0)
									: this.listTimeInEntries.get(0);
							timeOut = bioPerEmployeeByDatesSortedList.get(bioPerEmployeeByDatesSortedList.size() - 1);
	...
	public EmployeeScheduleUploading[] setTimeEntriesWithLunch(EmployeeScheduleUploading timeIn,
		...
		// CASES - for debugging - each bio entries

		// if same lunch in & time out, decide which entry to clear
		if ((lunchIn != null && timeOut != null) && timeOutReference.getTime() != null
				&& (lunchIn.getTimeEntry() != null && timeOut.getTimeEntry() != null)) {
			if (lunchIn.getTimeEntry().equals(timeOut.getTimeEntry())
					&& lunchIn.getTimeEntry().before(timeOutReference.getTime())) {
				timeOut = null;
			} else if (lunchIn.getTimeEntry().equals(timeOut.getTimeEntry())
					&& (lunchIn.getTimeEntry().equals(timeOutReference.getTime())
							|| lunchIn.getTimeEntry().after(timeOutReference.getTime()))) {
				lunchIn = null;
			}
		}
		...
		// if same timeIn & lunchOut, decide which entry to clear
		if (timeIn != null && lunchOut != null) {
			if (timeIn.getTimeEntry().equals(lunchOut.getTimeEntry())) {
				if (timeIn.getTimeEntry().before(lunchOutReference.getTime())) {
					lunchOut = null;
				} else if (timeIn.getTimeEntry().after(lunchOutReference.getTime())
		...
		// move lunchOut entry to LunchIn if lunchOut is null ALWAYS IN ALL CASE
